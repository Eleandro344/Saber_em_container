...
@csrf_exempt
def registrar_entrega_parcial(request):
    if request.method != 'POST':
        return JsonResponse({'erro': 'Método não permitido'}, status=405)
    ...
    try:
        ...
        numero_dominio = body.get('numero_dominio')
        nova_data_entrega = body.get('nova_data_entrega')
        texto_entrega_parcial = body.get('texto_entrega_parcial')
        
        if not numero_dominio or not nova_data_entrega or not texto_entrega_parcial:
            return JsonResponse({'erro': 'Parâmetros incompletos'}, status=400)
        ...
        return JsonResponse({
            'mensagem': 'Entrega parcial registrada com sucesso', 
            'competencia': proxima_entrega
        })
    
    except Exception as e:
        print(f"Erro ao registrar entrega parcial: {str(e)}")
        return JsonResponse({
